    ...
    def __init__(self, dna_sequence_length, dna_subsequence_length, number_of_true, number_of_false, seed=42):
        ...
        while len(self.dna_false_subsequences) < self.number_of_false:
            ...
            if false_subsequence_tuple not in false_subsequences_set:
                ...
                if not self.encoder.is_included(torch.tensor(false_subsequence, dtype=torch.long)):
        ...
        self.dna_false_subsequences = torch.stack([torch.tensor(subseq, dtype=torch.long) for subseq in self.dna_false_subsequences])
        ...
        index_tensor = torch.tensor(random.sample(numbers, number_of_true), dtype=torch.long)
